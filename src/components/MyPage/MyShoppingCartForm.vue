        ...
        <div class="sticky-nav">
          <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; width: 800px; margin: 0 auto;">
            ...
            <div class="order-btn-container">
              <v-btn class="orderBtn"  @click="selectPurchase">
  ...
  toggleAll(value) {
    ...
    for (let i=0; i < this.cartItems.length; i++ ) {
      if (this.checkedValues.includes(this.cartItems[i].cartItemId)) {
          selectedItems.push(this.cartItems[i])
      }
    }
    this.$store.commit('ordercartModule/REQUEST_ORDER_INFO_FROM_SPRING',
        { orderSave : { directOrderCheck: false, selectedItems: selectedItems, totalPrice: this.totalPrice }})
      console.log(this.$store.state.ordercartModule.orderList)
      if(selectedItems.length > 0 ) {
        console.log(this.$store.state.ordercartModule.orderList+"::: 00000000000")
        alert("주문페이지 이동.")
        await this.$router.push({ name: 'PaymentInfoPage' })
      }  else {
        alert("선택한 상품이 없습니다")
      }
